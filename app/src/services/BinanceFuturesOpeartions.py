...
class BinanceFuturesOps(BinanceClient):
    ...
    def checkWalletBalance(self, params):
        ...
        if self.is_authentic == True:
            ...
            result = []
            if self.quote_asset:
                res = [{"asset": item["asset"],"balance" :item["balance"]}for item in balances if item["asset"] == self.quote_asset]          
                result.extend(res)

            if self.base_asset:
                res = [{"asset": item["asset"],"balance" :item["balance"]}for item in balances if item["asset"] == self.base_asset]
                print(res)
                result.extend(res)
    ...
    def changePositionMode(self, params):
        mode = self.futures_change_margin_type(**params)
        return mode
    ...
    def getStopPrice(self, percentVal):
        stopPrice = percentVal
        return (self.round_decimals_down(stopPrice, self.pricePrecision))
    ...
    def LimitOrder(self, params):
        ...
        if params["reduceOnly"] == True:
            return {"timeInForce": self.timeInForce, "price": price, "reduceOnly": True, "quantity": quantity}
        else:
    ...
    def StopOrder(self, params):
        """Process the values of take profit Limit order"""
        stopPrice = 0
        if params["side"] == 'BUY' or params["side"] == 'Buy':
            stopPrice = self.getStopPrice(params['stopPrice'])
        if params["side"] == "SELL" or params["side"] == 'Sell':
            stopPrice = self.getStopPrice(params['stopPrice'])
    ...
    def sendOrder(self, params):
        print("ORDER DETAILS PARAMS",params)
