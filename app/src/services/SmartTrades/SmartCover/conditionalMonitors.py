"""
Smart Cover conditonal monitors
@Param take profit(s)
@Param stop loss
@Param trailing stop
...
def trailingSellMonitor(exchangeData, currentPrice, trailingSell):
    print('Checking the trailing Sell - SMART COVER')
    ...
    trailingSet = trailingSell["trailing_sell_price"]
    tempMarketPrice = float(currentPrice) # The current price now on binance at this instance
    trailingSellTarget = float(trailingSet) #Percentage to trail order set price
    ...
    if priceComparisonList == None:
        ...
        trailingSellLimit = ((100 - trailingSellTarget)/100) * tempMarketPrice

        priceComparisonList = [
            float(trailingSellLimit), float(tempMarketPrice)]
    ...
    # Here we check if the currentPrice, in our case sell is above the previous marketPrice we are trailing(that is price N-1)
    if currentPrice > tempMarketPrice:
        ...
        # now move the trailingSellLimit by the same percentage change
        trailingSellLimit = ((100 + perc_market_change)/100) * \
            priceComparisonList[0]

        priceComparisonList[0] = float(trailingSellLimit)
