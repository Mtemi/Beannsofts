...
class DeleteExchange(Resource):
    ...
    def post(self, user):
        ...
        userId = user.id
        exch_name = request.json['exch_name']
        # check if exchange exists before delete
        exchange_exists = get_exchange_by_name(exch_name)
        if not exchange_exists:
            return {
                "status": 404,
        ...
        if exchange == "binance":
            # binance_service = BinanceOps(api_key, api_secret)
            # if binance_service.authenticateKeys() == True:
            # no need to verify the keys just delete



            db.session.query(ExchangeModel).filter(
                ExchangeModel.user_id == userId, ExchangeModel.exchange_name == exch_name).delete()
            db.session.commit()
            return {"message":"API data deleted successifully", "status":200}
